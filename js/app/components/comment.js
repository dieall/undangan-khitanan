...
export const comment = (() => {
    ...
    const show = () => {
        if (!comments) {
            return Promise.resolve({ data: { lists: [], count: 0 } });
        }
    ...
    const deleteAll = async (button) => {
        if (!util.ask('Yakin ingin menghapus semua komentar?')) {
            return;
        }
        ...
        if (!session.isAdmin()) {
            ...
            comments.innerHTML = onNullComment();
            util.notify('Semua komentar telah dihapus dari tampilan.').success();
            btn.restore();
            return;
        }

        const previousPer = pagination.getPer();

        pagination.setPer(9999);
        pagination.reset();

        await show().catch(() => {});

        const topLevelComments = Array.from(document.querySelectorAll('#comments > div'));
        for (const commentContainer of topLevelComments) {
            const deleteButton = commentContainer.querySelector('button[onclick="undangan.comment.remove(this)"]');
            if (!deleteButton) {
                continue;
            }

            const own = deleteButton.getAttribute('data-own');
            ...
            try {
                const status = await request(HTTP_DELETE, '/api/comment/' + own)
                    .token(session.getToken())
                    .send(dto.statusResponse)
                    .then((res) => res.data.status);
